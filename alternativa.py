...
class kamailio:
    ...
    def set_state(self, user, state):
        ...
        KSR.hdr.add("State {user}", str(name))
    ...
    def remove_user_state(self, user):
        if user in user_states:
            ...
            KSR.hdr.add("State {user}", str("State {user} removed"))
    ...
    def ksr_request_route(self, msg):
        ...
        if (msg.Method == "REGISTER"):
            ...
            if  KSR.pv.get("$tu").endswith("@acme.pt"):
        ...
        if (msg.Method == "INVITE"):
            ...
            if "acme.pt" in KSR.pv.get("$fd"):
                
                #Conference call
                if "sip:conference@acme.pt" in KSR.pv.get("$tu"):
                ...
                if KSR.registrar.lookup("location") == 1:
                    ...
                    if self.get_state(KSR.pv.get("$tu")) == 1:
                    ...
                    elif self.get_state(KSR.pv.get("$tu")) == 2:
                        KSR.info(KSR.pv.get("$tu") + " is on a Conference.\n")
                        ...
                        if(msg.Method == "INFO"):
                            info_body = KSR.textops.get_body()
                            if "DTMF" in info_body:
                                dtmf_value = info_body.strip()
                                if dtmf_value == 0:
                                    KSR.pv.sets("$ru", "sip:conference@127.0.0.1:5090")
                                    KSR.info("Calling to Conference. Forwarding call to: " + KSR.pv.get("$ru") + "\n")
                                    self.set_state(KSR.pv.get("$fu"), 2)
